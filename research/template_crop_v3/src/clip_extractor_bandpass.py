...
def extract_centered_zero_padded(
    ...
    when_short: Literal["pad", "extend"] = "extend",
    bandpass: bool = True
) -> Tuple[np.ndarray, Dict]:
    """build a clip centered on the annotation, with options when the segment is shorter than target.

    behavior
    - if annotated segment is shorter than target:
        * when_short="pad": return zeros + segment + zeros, centered (original behavior)
        * when_short="extend": return a centered window from the original signal using surrounding context;
          may include audio outside [t_min, t_max]. if the recording bounds prevent a full window, pad the remainder.
    ...
    seg_end = min(rec_len, int(round(t_max * sr)))
    ...
    meta = {
        ...
        "pad_left": 0,
        "pad_right": 0,
        "context_left": 0,   # samples of context included to the left of [t_min, t_max]
        "context_right": 0,  # samples of context included to the right of [t_min, t_max]
        "start_in_recording": None,  # start index used in original recording y
        "end_in_recording": None,    # end index (exclusive) used in original recording y before padding
    }

    if samples_target <= 0:
        # guard against non-positive targets
        return np.zeros(0, dtype=np.float32), meta

    if seg_len >= samples_target:
        # crop centered within the segment
        center_in_seg = seg_len // 2
        half = samples_target // 2
        start_in_seg = max(0, center_in_seg - half)
        end_in_seg = start_in_seg + samples_target
        if end_in_seg > seg_len:
            end_in_seg = seg_len
            start_in_seg = end_in_seg - samples_target
        # map to recording indices
        start = seg_start + start_in_seg
        end = start + samples_target
        clip = y[start:end]
        meta.update({
            "method": "crop",
            "start_in_recording": int(start),
            "end_in_recording": int(end),
        })
    else:
        if when_short == "pad":
            ...
            meta.update({
                ...
                "start_in_recording": int(seg_start),
                "end_in_recording": int(seg_end),
            })
        elif when_short == "extend":
            # center on the segment center and extend the window into context within the recording
            seg_center = (seg_start + seg_end) // 2
            half = samples_target // 2
            start = seg_center - half
            end = start + samples_target

            # clamp to recording and shift the window if it overflows
            if start < 0:
                end = min(rec_len, end - start)  # shift right
                start = 0
            if end > rec_len:
                shift = end - rec_len
                start = max(0, start - shift)    # shift left
                end = rec_len

            # slice available audio
            clip = y[start:end]
            used_len = len(clip)

            # compute context relative to annotation window
            context_left = max(0, seg_start - start)
            context_right = max(0, end - seg_end)

            # pad only if the recording is too short to provide full window
            pad_left = 0
            pad_right = 0
            if used_len < samples_target:
                rem = samples_target - used_len
                # decide padding side(s) based on which boundary constrained us
                if start == 0 and end == rec_len:
                    # recording shorter than target: split padding to keep centered
                    pad_left = rem // 2
                    pad_right = rem - pad_left
                elif start == 0:
                    # constrained on left edge
                    pad_left = rem
                elif end == rec_len:
                    # constrained on right edge
                    pad_right = rem
            ...
            raise ValueError('when_short must be "pad" or "extend"')
